package org.mangui.jwplayer.media {
    ...
    public class HLSProvider extends MediaProvider {
        ...
        private function _errorHandler(event : HLSEvent) : void {
            Log.warn("HLS Error event received, dispatching error to jwplayer:" + event.error.code + "," + event.error.msg);
        ...
        private function _manifestHandler(event : HLSEvent) : void {
            ...
            if (_hls.type == HLSTypes.VOD || _levels[_hls.startlevel].duration > _seekInLiveDurationThreshold) {
            ...
            if (item.start != 0) {
                _hls.stream.play(null, item.start);
            } else {
                _hls.stream.play();
            }
        };
        ...
        override public function initializeMediaProvider(cfg : PlayerConfig) : void {
            ...
            for (var object : String in cfg) {
                ...
                if (subidx != -1) {
                    if (object.indexOf("live_seekdurationthreshold") != -1) {
                        _seekInLiveDurationThreshold = cfg[object] as Number;
                    } else {
