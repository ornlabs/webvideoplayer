package org.mangui.HLS.streaming {
    ...
    public class FragmentLoader {
        ...
        private function _fragParsingProgressHandler(audioTags : Vector.<Tag>, videoTags : Vector.<Tag>) : void {
             ...
             */
            if (( _fragment_first_loaded || (_manifest_just_loaded && HLSSettings.startFromLevel != -1) ) && !_pts_loading_in_progress) {
                // compute min/max PTS
                var min_pts : Number;
                var max_pts : Number;
                ...
                if (min_pts != Number.POSITIVE_INFINITY && max_pts != Number.NEGATIVE_INFINITY) {
                    var frag : Fragment = _levels[_level].getFragmentfromSeqNum(_seqnum);
                    var min_offset : Number;
                    var max_offset : Number;
                    if (frag) {
                        min_offset = frag.start_time + pts_start_offset / 1000;
                        max_offset = frag.start_time + pts_end_offset / 1000;
                    } else {
                        min_offset = 0;
                        max_offset = 0;
                    }
